  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { document_id } = await apiClient.generateLegalDocument({
        ...
        purpose: summary.trim() || `Standard ${selectedTemplate.name} for ${counterparty.trim() || partyName.trim()}.`,
      ...
      onSubmit({
        ...
        summary: summary.trim() || `Standard ${selectedTemplate.name} generated for ${counterparty.trim() || partyName.trim()}.`,
        ...(result as any),
      });
          ...
          <form onSubmit={handleSubmit} className="p-6 pt-2 space-y-5">
            {activeTab === 'configure' ? (
              <>
                ...
                {/* 3b. Party Name & Location (for API wiring) */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-foreground-soft mb-1.5">
                ...
                >
                  {isSubmitting ? (
                    <>
                      ...
                      <span>{pollingStatus || 'Generating Draft...'}</span>
                    </>
                  ) : (
                    <>
