    ...
    async function fetchInitialData() {
      try {
        ...
        if (jobsRes.status === 'fulfilled') {
          const fetchedRoles: Role[] = jobsRes.value.map((job: any) => ({
            id: job.id,
            title: job.role,
            department: job.business_id,
            candidatesCount: 0,
            currentStage: 'Active',
            lastActivity: new Date(job.created_at).toLocaleDateString(),
            status: job.status,
            targetDate: 'TBD',
          ...
          jobsRes.value.forEach((job: any) => {
            if (job.status === 'done' && job.approval_status === 'pending_review') {
              dynamicApprovals.push({
                id: job.id,
                department: 'hiring',
                departmentLabel: 'Hiring',
            ...
            dynamicActivities.push({
              ...
              department: 'hiring',
              departmentLabel: 'Hiring',
              action: job.status === 'done' ? 'Role JD Drafted' : 'Role Created',
              shortDescription: `The agent ${job.status === 'done' ? 'finished drafting' : 'started processing'} the ${job.role} role.`,
              actor: 'FORGE Talent Engine'
            });
          });
        }
        ...
        if (docsRes.status === 'fulfilled') {
          const fetchedDocs: LegalDocument[] = docsRes.value.map((doc: any) => ({
            id: doc.id,
            ...
            owner: 'Founder',
            lastUpdated: new Date(doc.created_at).toLocaleDateString(),
            governingLaw: doc.input_context?.location || 'TBD',
            effectiveDate: 'TBD',
          ...
          docsRes.value.forEach((doc: any) => {
            if (doc.status === 'done' && doc.approval_status === 'pending_review') {
              dynamicApprovals.push({
                id: doc.id,
                department: 'legal',
                departmentLabel: 'Legal',
                ...
                shortDescription: doc.input_context?.purpose || 'Legal document drafted by agent.',
                timeAgo: new Date(doc.created_at).toLocaleDateString(),
                status: 'ready',
                statusLabel: 'Ready for review',
                priority: 'normal',
                whatForgePrepared: {
            ...
            dynamicActivities.push({
              ...
              department: 'legal',
              departmentLabel: 'Legal',
              action: doc.status === 'done' ? 'Document Drafted' : 'Document Requested',
              shortDescription: `The agent ${doc.status === 'done' ? 'finished drafting' : 'started generating'} a ${doc.document_type} document.`,
              actor: 'FORGE Legal Guard'
            });
          });
        }
